...
<details>
<summary>🔒 <strong>For facilitators only</strong>: example + starter skill source</summary>

Attendees play in their own shared repo (`qa-shared-tools`) and follow the [Test Case Standard](https://aspenware.atlassian.net/wiki/spaces/QA/pages/4137582614/Test+Case+Standard) in Confluence. The **`QUAL-4510` example** and the **starter `write-test-cases` skill** come from the session materials; the **training repo** ships both: [github.com/tracedwax/ac-training-repo](https://github.com/tracedwax/ac-training-repo) ([zip](https://github.com/tracedwax/ac-training-repo/archive/refs/heads/main.zip)). Drop them into the attendee's branch if needed; don't send attendees to that repo.
</details>
...
### Step 3: Point Claude at the team standard
> *"Follow our team's Test Case Standard at https://aspenware.atlassian.net/wiki/spaces/QA/pages/4137582614/Test+Case+Standard (pull it from Confluence, or I'll paste it). Match the format of the `QUAL-4510` example my facilitator gave me, and show me how each AC maps to a test."*
