...
> **Materials:** **Product** works in their **own folder**: needs **OpenSpec** (Claude installs live) and the [`openspec-process.md`](openspec-process.md) page dropped in; tickets go into the attendee's **own Jira project** (the **Mocking Project** `MP` is a shared sandbox backup, https://aspenware.atlassian.net/jira/software/projects/MP/boards). **QA** works in **`qa-shared-tools`** (ADO); their [Test Case Standard](https://aspenware.atlassian.net/wiki/spaces/QA/pages/4137582614/Test+Case+Standard) is in Confluence, and the `QUAL-4510` example and starter skill come from the session materials.
